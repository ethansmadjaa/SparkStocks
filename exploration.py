...
def analyze_ma_signals(df: DataFrame, short_period: int = 5, long_period: int = 20) -> DataFrame:
    ...
    df = df.withColumn("signal",
        when((col(signal_col) > 0) & (col("prev_signal") < 0), "Golden Cross (Buy)")  # Short MA crosses above Long MA
        .when((col(signal_col) < 0) & (col("prev_signal") > 0), "Death Cross (Sell)")  # Short MA crosses below Long MA
